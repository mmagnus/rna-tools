...
def get_parser():
    ...
    parser.add_argument('-a',"--rna_alignment_fn", help="rna alignemnt with the extra guidance line, e.g. test_data/rp14sub.stk", required=True)
    ...
    parser.add_argument('-n',"--target_name", help="target name in the alignment, used to map target on the alignment, e.g. target, ade, rp14 etc.", required=True)
    ...
    parser.add_argument('-g', '--group_name', help='name given group of structure, helps to analyze results', default='')
    return parser
...
def calc_evo_rmsd(targetfn, target_name_alignment, files, mapping, rna_alignment_fn, group_name='', output_fn=None):
    ...
    rnastruc = open(mapping).read().replace('\n','').strip().split('|') # 'target:rp14_farna_eloop_nol2fixed_cst|X:X'
...
if __name__ == '__main__':
    ...
    df = calc_evo_rmsd(opts.target, opts.target_name, opts.files, opts.mapping_fn, opts.rna_alignment_fn, opts.group_name, opts.output_fn)
    print df
