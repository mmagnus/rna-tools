...
function (shades1, shades2, shadeIndex, xA, yA, zA, dxBA, dyBA, dzBA, clipped) {
this.x1t = xA;
this.x2t = xA + dxBA;
this.y1t = yA;
this.y2t = yA + dyBA;
...
this.plotLineClippedA (shades1, shades2, shadeIndex, xA, yA, zA, dxBA, dyBA, dzBA, clipped, 0, 0);
}, "~A,~A,~N,~N,~N,~N,~N,~N,~N,~B");
...
this.plotLineClippedBits (shades1, shades2, shadeIndex, xA, yA, zA, dxBA, dyBA, dzBA, 0, 0);
}, "~A,~A,~N,~N,~N,~N,~N,~N,~N,~B");
...
Clazz.defineMethod (c$, "plotLineClippedA", 
 function (shades1, shades2, shadeIndex, x, y, z, dx, dy, dz, clipped, run, rise) {
var zbuf = this.g3d.zbuf;
var width = this.g3d.width;
var runIndex = 0;
if (run == 0) {
...
var argb = argb1;
if (argb != 0 && !clipped && offset >= 0 && offset < offsetMax && z < zbuf[offset]) this.g3d.addPixel (offset, z, argb);
if (dx == 0 && dy == 0) {
return;
}var xIncrement = 1;
var yOffsetIncrement = width;
...
}}, "~A,~A,~N,~N,~N,~N,~N,~N,~N,~B,~N,~N");
Clazz.defineMethod (c$, "plotLineClippedBits", 
 function (shades1, shades2, shadeIndex, x, y, z, dx, dy, dz, run, rise) {
var zbuf = this.g3d.zbuf;
var width = this.g3d.width;
var runIndex = 0;
if (run == 0) {
